...
function getBranch() {
  try {
    return shell.exec('git rev-parse --abbrev-ref HEAD').trim();
  } catch (e) {
...
function checkUpstream(remote, branch) {
  try {
    shell.exec(`git log ${remote}/${branch}..HEAD`, {'stdio': 'ignore'}).trim();
    return true;
  } catch (e) {
    return false;
  }
...
function getCommitedFiles(remote, branch, upstream) {
  try {
    ...
    let cmd;
    if (upstream) {
      commits = shell.exec(`git log ${remote}/${branch}..HEAD`)
        .trim().split('\n').filter(isTruthy);
      cmd = `git diff --stat --cached ${remote}/${branch}`;
    } else {
      commits = shell.exec('git log --branches --remotes --simplify-by-decoration --oneline --format=format:%H')
        .trim().split('\n').filter(isTruthy);
    ...
    return shell.exec(cmd).trim().split('\n').filter(isTruthy);
  } catch (e) {
    console.error(e);
    process.exit(1);
  }
...
function getStagedFiles() {
  const repoPath = getRootDir();
  process.chdir(repoPath);
  const stagedFiles = shell.exec('git diff --cached --name-only')
    .trim().split('\n').filter(isTruthy);
...
function getUsername() {
  username = username || shell.exec('git config user.name').trim();
  return username;
}
