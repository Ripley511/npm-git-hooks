...
/**
 * @constructor NoTaskError(name, operation)
 * @desc ErrorType constructor in case no tasks are found
 * @param {String} name
 * @param {String} operation
 */
function NoTaskError(name, operation) {
  this.name = 'NoTaskError';
  this.message = `${colors.inverse('npm-git-hooks')} ${colors.yellow.inverse('INFO')} No tasks were found for operation ${operation} in project ${name}, moving on...`;
...
function RunTaskError(task, project) {
  ...
  this.message = `${colors.inverse('npm-git-hooks')} ${colors.red.inverse('FAIL')} git operation not permitted because task ${this.task} failed for project ${this.project}`;
  this.stack = (new Error()).stack;
}
 ...
 */
function successCallback(pkg, tasks) {
  console.log(`${colors.inverse('npm-git-hooks')} ${colors.green.inverse('SUCCESS')} Tasks '${tasks.join(', ')}' in project ${pkg.name}`);
}
